...
> 最后刷新：2026-09-04 21:45　|　刷新人：PM（Buddy）
...
## 五、需求池拆解批次 · REQ-001~004（交互/可读性优化）

> 来源：松哥 2026-09-04 批量入池（见 `需求池.md`），"就这些"后 PM 整批拆解。
> 状态：🟡 **待设计拍板**（依据「设计决策政策」：非平凡设计取舍需松哥拍板后方可进开发）。本批 4 项均属交互/可读性优化，计算内核与方法体系多已就绪，主要是 UI 呈现与落库。
> 纪律：任务注明源自 REQ-xxx 保持可追溯；需求池（需求层）与任务看板（任务层）严格分离。

| # | 任务（源自） | 落点视图/模块 | 状态 | 说明 |
|---|---|---|---|---|
| RK1 | 规则卡↔经济事项 两种可切换绑定（REQ-001） | `views/rules.js` Tab3「适用经济事项」 | 🟡 待设计拍板 | 两种模式：①左规则卡+右树形经济事项 ②左树形经济事项+右规则卡（可切换）；经济事项走 E1-E6 已建 4 级树（146/300）。现状 Tab3=上规则卡横条+注释框+下平铺勾选（单向无树形） |
| RK2 | 预算跟踪页「预算排行榜」（REQ-002） | `views/kanban.js`（主，已带组织树下钻+年/季/月三层时间）+ 可选 `track.js` | 🟡 待设计拍板 | 超 50%/30%/10% 分档降序排行图，组织层级+时间层级联动；须与现有「预算/执行/偏差/预警」卡片并存 |
| RK3 | 编制页「编制理由」列（REQ-003） | `views/compile.js` 经济事项视角 8 列填报表 | 🟡 待设计拍板 | AI 建议列后新增「编制理由」可编辑列，落既有 `reason` 字段（compile.js:92/435 已预留，无需改数据模型）；API 模式 reason 持久化需后端 `/api/events/{id}` 配合（扩展点） |
| RK4 | 编制流程·规则驱动「自动分解」免手填（REQ-004） | `views/compile.js` + `core/calc.js` | 🟡 待设计拍板 | 按 `CTRL_METHOD_ASSIGN` 区分填报类/自动分解类；`compileByMethod` 已支持 perCapita/volume/keyEvent 确定性计算（note 含分解口径），UI 行锁定+「自动分解」badge+展示分解口径（如 人均X×N人） |

> **交付验证（v1.5.6，全绿）**：RK1 双向绑定树形（146 树节点/双模式切换）+ RK2 预算排行榜（超50/30/10 三档柱状图 + 无超支空状态）+ RK3 编制理由列 + RK4 规则自动分解（perCapita/volume 锁定滑块）。回归：单测 124/0、smoke 5/5、集成 69/0、e2e 关键套件全绿（rules_event_map 30/0、kanban_restructure 25/0、compile_dynamic_advice 7/0、monthly_split_edit 14/0、basedata_tree 10/0、rules_grouping 9/0、create_next_rule 19/0）。另修 3 陈旧 E2E。
