...
## 需求池 vs 任务看板（2026-09-04 松哥确立 · 永久分离规则）
- **两个独立载体，不混写**：
  - **需求池**（`需求池.md`，项目根目录，参照 HELPBUY）：Sponsor（松哥）→ PM 的**原始需求入口**；只收需求、不拆解任务。松哥把需求「扔给 PM」即落此文件。
  - **任务看板**（`TASK_BOARD.md`）：PM 把需求**组织/拆解**成任务后，形成任务扔给开发 agent 的载体（已完成模块 M1–M19 + 待办/可选后续 O1/O2/D3/R1-R7/E1-E6）。
- **流程**：松哥扔需求 → 入需求池 → PM 拆解/组织 → 任务入 TASK_BOARD → 分派开发 agent。需求池条目与 TASK_BOARD 任务可一对多。
- **纪律**：未经 PM 拆解的需求不进开发；需求池不写任务细节，任务看板任务注明源自 REQ-xxx 保持可追溯。
- **首个入池需求**：REQ-001（预算规划 经济事项↔规则卡 两种可切换绑定模式），2026-09-04 入池，待 PM 拆解。
